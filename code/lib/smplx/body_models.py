...
from .lbs import (lbs, blend_shapes, vertices2joints)

from .vertex_ids import vertex_ids as VERTEX_IDS
from .utils import (Struct, to_np, to_tensor, Tensor, Array, SMPLOutput,
                    SMPLHOutput, SMPLXOutput, MANOOutput,
...
class SMPL(nn.Module):
    ...
    def __init__(self,
        ...
        if data_struct is None:
            ...
            with open(smpl_path, 'rb') as smpl_file:
                data_struct = Struct(
                    **pkl.load(smpl_file, encoding='latin1'))
    ...
    def forward(self,
        ...
        if displacement is not None:
            vertices, joints_smpl, pose_feature, T = lbs(
                betas, full_pose, v_template + displacement, self.shapedirs,
                self.posedirs, self.J_regressor, self.parents,
                self.lbs_weights)
        else:
            vertices, joints_smpl, pose_feature, T = lbs(
                betas, full_pose, v_template, self.shapedirs, self.posedirs,
        ...
        if data_struct is None:
            ...
            if ext == 'pkl':
                with open(smplh_path, 'rb') as smplh_file:
                    model_data = pkl.load(smplh_file, encoding='latin1')
            elif ext == 'npz':
...
class SMPLX(SMPLH):
    ...
    def forward(self,
        ...
        if displacement is not None:
            vertices, joints_smpl, pose_feature, T = lbs(
                shape_components, full_pose, v_template + displacement,
                shapedirs, self.posedirs, self.J_regressor, self.parents,
                self.lbs_weights)
        else:
            vertices, joints_smpl, pose_feature, T = lbs(
                shape_components, full_pose, v_template, shapedirs,
                self.posedirs, self.J_regressor, self.parents,
                self.lbs_weights)
...
class MANO(SMPL):
        ...
        if data_struct is None:
            ...
            if ext == 'pkl':
                with open(mano_path, 'rb') as mano_file:
                    model_data = pkl.load(mano_file, encoding='latin1')
            elif ext == 'npz':
    ...
    def forward(self,
        ...
        if displacement is not None:
            vertices, joints_mano, pose_feature, T = lbs(
                betas, full_pose, v_template + displacement, self.shapedirs,
                self.posedirs, self.J_regressor, self.parents,
                self.lbs_weights)
        else:
        ...
        #     joints = self.joint_mapper(joints_mano)

        if apply_trans:
            joints_mano = joints_mano + transl.unsqueeze(dim=1)
            # joints = joints + transl.unsqueeze(dim=1)
            vertices = vertices + transl.unsqueeze(dim=1)

        output = MANOOutput(vertices=vertices if return_verts else None,
                            joints=joints_mano if return_verts else None,
                            betas=betas,
                            global_orient=global_orient,
                            hand_pose=hand_pose,
...
def create(model_path: str,
           model_type: str = 'smpl',
           **kwargs) -> Union[SMPL, SMPLH, SMPLX, MANO]:
